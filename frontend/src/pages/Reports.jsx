...
export default function Reports() {
  ...
  const bookCols = [
    ...
    { label: 'Type', key: 'type' },
    { label: 'Available', key: 'available', render: x=>x.available ? 'Yes' : 'No' }
  ];
  const memberCols = [
    { label: 'Member Name', key: 'memberName' },
    { label: 'Membership #', key: 'membershipNumber' },
    { label: 'Active', key: 'active', render: x=>x.active?'Yes':'No' },
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 mb-6">
        <button onClick={()=>setTab('transactions')} className={`px-4 py-2 rounded ${tab==='transactions'?'bg-blue-500 text-white':'bg-gray-200'}`}>Transactions</button>
        <button onClick={()=>setTab('books')} className={`px-4 py-2 rounded ${tab==='books'?'bg-blue-500 text-white':'bg-gray-200'}`}>Books</button>
        <button onClick={()=>setTab('members')} className={`px-4 py-2 rounded ${tab==='members'?'bg-blue-500 text-white':'bg-gray-200'}`}>Members</button>
      </div>
