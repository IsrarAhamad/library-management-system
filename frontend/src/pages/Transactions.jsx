      ...
      <div className="w-full max-w-[768px]">
        ...
        <div className="flex gap-4 mb-4">
          <button
            className={`px-4 py-2 rounded ${
              mode === "issue" ? "bg-blue-600 text-white" : "bg-gray-200"
            }`}
          ...
          <button
            className={`px-4 py-2 rounded ${
              mode === "return" ? "bg-blue-600 text-white" : "bg-gray-200"
            }`}
        ...
        {!books.length && (
          <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded">
            No books available to issue. Please add books in Maintenance.
          </div>
        )}

        {!filteredMemberships.length && (
          <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded">
            No membership linked to your profile. Contact an admin.
          </div>
        )}
        <TransactionsForm
          mode={mode}
        ...
        />
        <div className="mt-8 bg-white border rounded-lg shadow-md p-5 w-full">
          <h3 className="font-bold mb-2">Recent Transactions</h3>
          <p className="text-sm text-gray-600 mb-3">
            Click a transaction while in Return mode to prefill the form.
          </p>
          ...
          <ul className="space-y-2">
            {recent
              ...
              .map((txn) => (
                <li
                  key={txn._id}
                  className="p-3 bg-gray-50 border rounded hover:bg-gray-100 cursor-pointer transition"
                  onClick={() => handleTxnClick(txn)}
                >
                  <div className="font-medium">
                    {txn.book?.title || "Book"} (
                    {txn.book?.serialNumber || "N/A"})
                  </div>
                  <div className="text-sm text-gray-600">
                    By {txn.user?.name || "User"} on{" "}
                    {txn.issueDate?.slice(0, 10)}
                  </div>
                </li>
              ))}
          </ul>
